...
<table border=1>
  ...
  <tr>
    <td>
      ...
      $order = "SELECT * FROM cust_info where cust_id='$id'";
      $result = mysql_query($order);
      $row = mysql_fetch_array($result);
      ?>
      <form name="Customer" method="post" action="includes/edit_data_customer.php">
      ...
      size="20" value="<? echo "$id"?>" readonly>
        ...
        <tr>
          <td>Phone 2</td>
          <td>
            <input type="text" name="phone_2" size="40" 
          value="<? echo "$row[phone_2]"?>">
          </td>
        </tr>
        <tr>
          <td>Address</td>
          <td>
            <input type="text" name="cust_address" size="40" 
          value="<? echo "$row[cust_address]"?>">
          </td>
        </tr>
        <tr>
          <td>Credit Limit</td>
