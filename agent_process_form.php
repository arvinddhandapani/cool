...
class ProcessForm
{
    ...
    function process()
    {
         ...
         */
	     ...
	     //Inserting into DB Agent Info
	         foreach($this->fields as $key => $field)
	     {
	         if ($key == 'agent_name')
	             $agent_name = $field;
	         elseif ($key == 'agent_phone')
	            $agent_phone = $field;
	         elseif ($key == 'agent_credit_limit')
	             $agent_credit_limit = $field;
	         elseif ($key == 'agent_lcr_limit')
	             $agent_lcr_limit = $field;
	     }



	     global $con;
	     $query_string = "INSERT INTO driver_info
	     (full_name, phone_1, credit_limit, lcr_limit)
	     VALUES
	     ('$agent_name', '$agent_phone', '$agent_credit_limit', '$agent_lcr_limit')";
