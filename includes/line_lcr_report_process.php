...
class ProcessForm
{
    ...
    function process()
    {
	 ...
	 echo $cust_id1;
	 if ($cust_id1 != ''){
	 $credit_report_get = "Select t1.custi_id_fk, t1.today_date, t2.full_name, t3.full_name, t1.total_case_out, t1.total_case_in from outwards_LCR As t1 INNER JOIN cust_info As t2 ON t1.custi_id_fk = t2.cust_id INNER JOIN driver_info As t3 ON t3.driver_id = t1.driver_id_fk where t1.driver_id_fk = ". $cust_id1;
	 
	 $get_sum_lcr_outstanding = "Select sum(total_case_out) - sum(total_case_in) AS result from outwards_LCR where driver_id_fk = ". $cust_id1;
	 ...
	 <a href= export_line_lcr.php?msg=<?php echo $cust_id1?>> Export </a>
	 ...
	 echo "<H1> There is no data to display </H1>";
 }
