...
class ProcessForm
{
    ...
    function process()
    {
     ...
     (custi_id_fk, driver_id_fk, today_date, total_case_in)
     VALUES
     ('$cust_id1', '$driver_id1', '$today_date', '$total_case_in')";
	 
	 $insert_credit = mysqli_query($con, $query_string);
	 $error = mysqli_error($con);
	 
  if ($insert_credit){
	  header('Location: ../LCR_collection.php?msg=LCR entered successfully');
  } else {
	  header('Location: ../LCR_collection.php?msg=There was an error inserting.. Please check the values');
  	echo $error;
  }
