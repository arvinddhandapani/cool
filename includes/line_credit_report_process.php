...
class ProcessForm
{
    ...
    function process()
    {
	 ...
	 $get_cust_id = $mysqli->prepare("select driver_id from driver_info WHERE full_name = ?");
	 ...
	 $get_cust_id_total = "select sum(amount) from outwards_credit WHERE bill_state = 0 and driver_id_fk = ". $cust_id1;
	 $get_cust_credit_sum = mysqli_query($con, $get_cust_id_total);
	 $error1 = mysqli_error($con);
	 echo $error1;
	 $relut = array();
	 while ($row1 = mysqli_fetch_row($get_cust_credit_sum)) {
		 $driver_total_outstanding = $row1[0];
	 }
	 
	 $credit_report_get = "Select t1.custi_id_fk, t1.date_out, t2.full_name, t3.full_name, t1.settlement_no, t1.bill_no, t1.amount from outwards_credit As t1 INNER JOIN cust_info As t2 ON t1.custi_id_fk = t2.cust_id INNER JOIN driver_info As t3 ON t3.driver_id = t1.driver_id_fk where t1.bill_state = 0 and t1.driver_id_fk = ". $cust_id1. " ORDER BY t1.date_out ASC";
	 //echo $credit_report_get;
	 ...
	 if ($get_cust_credit_report){
		 ...
		 <P>The total outstanding for line agent <?php echo $cust_name ?> is <?php echo $driver_total_outstanding ?>
	 ...
	 <a href= export_line.php?msg=<?php echo $cust_id1?>> Export </a>
