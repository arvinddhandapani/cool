...
<body>
    ...
    <div class="page-wrapper">
	 ...
	 $get_sum_credit_outstanding = "Select sum(amount) from outwards_credit where bill_state = 0";
	 $sum_query = mysqli_query($con, $get_sum_credit_outstanding);
	 $error = mysqli_error($con);
	 $get_sum_credit_outstanding1 = mysqli_fetch_row($sum_query);
	 $get_sum_credit_outstanding = $get_sum_credit_outstanding1[0];
	 
	 $get_user_details = "Select t1.custi_id_fk, t2.full_name, t2.phone_1, t2.phone_2, sum(t1.amount) from outwards_credit As t1 INNER JOIN cust_info As t2 ON t1.custi_id_fk = t2.cust_id where t1.bill_state = 0 group by t1.custi_id_fk ";
 	...
 	<tbody>
		...
		while ($row = mysqli_fetch_row($credit_query)) {
			echo "<tr>";
			//echo "<td>". $row[0]. "</td>";
			echo "<td><a href= 'credit_report_customer.php?msg=". $row[1]. "'>". $row[1]. "</a></td>'";
			echo "<td>". $row[2]. "</td>";
			echo "<td>". $row[3]. "</td>";
			echo "<td>". $row[4]. "</td>";
			echo "</tr>";
