...
class ProcessForm
{
    ...
    function process()
    {
        ...
        //Inserting into DB Customer Info
        foreach($this->fields as $key => $field)
        {
            if ($key == 'cust_name')
                $cust_name = $field;
            elseif ($key == 'cust_phone1')
            $cust_phone1 = $field;
            elseif ($key == 'cust_phone2')
            $cust_phone2 = $field;
            elseif ($key == 'cuts_address')
            $cuts_address = $field;
            elseif ($key == 'cust_credit_limit')
            $cust_credit_limit = $field;
            elseif ($key == 'cust_lcr_limit')
            $cust_lcr_limit = $field;
        }
        
        
        
        global $con;
        $query_string = "INSERT INTO cust_info
        (full_name, phone_1, phone_2, cust_address, credit_limit, lcr_limit)
        VALUES
        ('$cust_name', '$cust_phone1', '$cust_phone2', '$cuts_address', '$cust_credit_limit', '$cust_lcr_limit')";
